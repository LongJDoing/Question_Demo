...
{
    float Len_v0v1 = v0.distance(v1);
    float Len_v1v2 = v1.distance(v2);
    float Len_v2v0 = v2.distance(v0);
    float a = Len_v0v1;
    float b = Len_v1v2;
    float c = Len_v2v0;
    float temp = 2*(a+b+c);
...
//if point in circle, then return empty vector
{
    ...
    tangentPointLeft=point + (dirPTLeft * disPT);
    
    tangentPointRight=point + (dirPTRight * disPT);
    vector<Vec2> rs;
    rs.push_back(tangentPointLeft);
    rs.push_back(tangentPointRight);
    return rs;
